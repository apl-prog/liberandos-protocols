...
if (window.__RDB_AUDIO_LOADED__) {
  ...
  // Resolve assets relative to THIS script file (not the page URL)
  ...
  async function initAudio() {
    ...
    // Load + decode with progress + clear failure reporting
    buffers.clear();
    for (let i = 0; i < STEMS.length; i++) {
      ...
      try {
        ...
        setStatus("ERROR");
        setOverlayMsg(`FAILED: ${s.label} (check file name/path)`);
        throw err;
      }
    }
  ...
  async function loadBuffer(url) {
    // Quick log so you can click URL in console
